#!/bin/bash
# build-lvgl.sh - Build Tasmota32 S3 LVGL firmware for ESP32S3-Geek
# Version: 15.0.1
# by Harald Kiessling
...
if [ ! -d "$TASMOTA_DIR" ]; then
    ...
    if [ "$CURRENT_VERSION" != "$TASMOTA_VERSION" ]; then
        ...
        git fetch --depth 1 origin tag "$TASMOTA_VERSION"
...
if [ -f "$CONFIG_DIR/user_config_override.h" ]; then
...
if "$PIO" run -e "$BUILD_ENV"; then
    ...
    if [ -f "$BUILD_DIR/firmware.bin" ]; then
        cp "$BUILD_DIR/firmware.bin" "$FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.bin"
        echo -e "${GREEN}✓ firmware.bin → tasmota32s3-lvgl-15.0.1.bin${NC}"
    fi
    
    if [ -f "$BUILD_DIR/firmware.factory.bin" ]; then
        cp "$BUILD_DIR/firmware.factory.bin" "$FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.factory.bin"
        echo -e "${GREEN}✓ firmware.factory.bin → tasmota32s3-lvgl-15.0.1.factory.bin${NC}"
    fi
    
    echo ""
    echo -e "${YELLOW}Firmware sizes:${NC}"
    ls -lh "$FIRMWARE_DIR"/tasmota32s3-lvgl-15.0.1*.bin 2>/dev/null | awk '{print "  " $9 ": " $5}'
    echo ""
    ...
    if command -v strings &> /dev/null; then
        ...
        if strings "$FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.bin" | grep -q "LVGL"; then
        ...
        if strings "$FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.bin" | grep -q "HASPmota"; then
        ...
        if strings "$FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.bin" | grep -q "tasmota32s3-lvgl"; then
    ...
    echo -e "  $FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.bin"
    echo -e "  $FIRMWARE_DIR/tasmota32s3-lvgl-15.0.1.factory.bin"
    echo ""
    ...
    echo -e "       write_flash -z 0x0 firmware/tasmota32s3-lvgl-15.0.1.factory.bin"
    echo ""
